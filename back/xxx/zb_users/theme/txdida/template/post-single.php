...
<div class="info-js xia15">
    <img src="{txdida_FirstIMG($article,$article,176,230)}" class="info-js-tu fl" />
    <h2>{$article.Title}</h2>
    <p><strong>发布时间：</strong>{$article.Time('Y年m月d日')} </p>
    <p><strong>视频标签：</strong>{foreach $article.Tags as $tag}<a href="{$tag.Url}">{$tag.Name}</a>&nbsp;&nbsp;{/foreach}</p>
    <p><strong>观看人数：</strong>{$article.ViewNums}</p>
    <p><strong>评论人数：</strong>当前有 {$article.CommNums} 人发表了看法 <a href="#comments" style="margin-left:10px;"><i class="iconfont fa-hand-o-right"></i> 我要评论</a></p>
    <div>
        <a href="#bfqk" class="bofang zse"><i class="iconfont icon-shipin2"></i> 开始播放</a>
        {if $article.Metas.xiazai}<a href="{$article.Metas.xiazai}" class="bofang fse" target="_blank"><i class="iconfont icon-xiazai"></i> 下载资源</a>{/if}
        {if $zbp->CheckPlugin('YtUser')}
        {if !$article.Favorite}
        <a href="javascript: void(0)" class="bofang fse" onclick="YtFavorite('add',{$article.ID},this);"><i class="iconfont icon-xihuan"></i> 收藏</a>
        {else}
        <a href="javascript: void(0)" class="bofang fse" onclick="YtFavorite('del',{$article.ID},this)"><i class="iconfont icon-xihuan"></i> 取消收藏</a>
        {/if}
        {/if}
    </div>
    <div class="clear"></div>
...
{if $zbp->Config('txdida')->listad4on=='1'}
<div class="ad xia15">{$zbp->Config('txdida')->listad4}</div>
{/if}
{if $zbp->Config('txdida')->sjgg4on=='1'}
<div class="xia15 dnwu">{$zbp->Config('txdida')->sjgg4}</div>  
{/if}


<div class="xia15">
    <h2 class="ybbt xia15"><strong>剧情介绍</strong></h2>
    <div class="info-zi">{$article.Content} {$zbp->Config('txdida')->fx}</div>
</div>
...
<div class="info-lb" id="bfqk">
    ...
    <div class="buy-form buy-lock mb20">
        ...
        {else}
        <p>此内容为vip可见内容，请先升级为本站的vip。</p>
        <a href="{$host}?Login" class="tx-btn zse">请先登录</a>
        {/if}
    </div>
    {/if}
