...
<div class="zh">
    ...
    <div class="user-right fr mb20">
        <div class="tx-box pd20">
            <div class="user-tab"><a href="{$host}?Upgrade" class="on">购买VIP</a><a href="{$host}?Integral">积分充值</a></div>
            <div class="pd20 bgh border mb20">
                <h3 class="f-18 mb10"><span class="mr10">你是：{$lang['user_level_name'][$user.Level]}</span> {if $user.Level < 5}到期时间：{$user.Vipendtime} {/if}</h3>
                <p class="f-hui">{if $zbp.Config('YtUser').readme_text}{$zbp.Config('YtUser').readme_text}{else}暂无说明{/if}</p> 
            </div>  
            <div class="data-box" >
